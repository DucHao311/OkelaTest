...
# Đảm bảo thư mục "results_data" tồn tại
output_dir = os.path.join(outputDir, "results_data")
...
for d in dataset_dicts:  
    ...
    plt.imsave(os.path.join(outputDir,
                            "results_data",
                            # (d["file_name"].split("/")[-1])[:-4]+'_gt'+".png"),
                            d["file_name"].split("/")[-1]+'_gt'+".png"),
                            vis.get_image()[:, :, ::-1])
    ...
    plt.imsave(os.path.join(outputDir, "results_data", d["file_name"].split("/")[-1]), v.get_image()[:, :, ::-1])
