...
date_stamp = date.today().strftime("%Y_%m%d")
...
parser.add_argument('--outputDir', type=str, required=True,
                    help="path to the output directory, note that this is the directory that stores the weights file, and the evaluation results will be stored in <outputDir>/results")
parser.add_argument('--weightsFile', type=str, default="model_final.pth",
                    help="name of the weights file, note that the absolute path is <outputDir>/<weightsFile>")
...
dataset_dicts = DatasetCatalog.get("amodal_coco_test")
print(f"\n annTestFile {annTestFile}")
print(f"\n imgTestFile {imgTestFile}")
...
cfg.DATASETS.TRAIN = ("amodal_coco_test",)
...
print(f"Initiate Amodal Evaluator ")
evaluator = AmodalEvaluator("amodal_coco_test", cfg, False,
                            output_dir=os.path.join(cfg.OUTPUT_DIR, "results", date_stamp))
print(f"Loading val loader")
val_loader = build_detection_test_loader(cfg, "amodal_coco_test")
print(f"Run inference on dataset")
